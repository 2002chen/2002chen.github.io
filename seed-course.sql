...
with chapter_seed(title, subtitle, description, level, position, cover_icon, estimated_minutes) as (
  values
    ('认识 Python 与开发环境', '先弄懂代码是什么', '不预设任何基础：从电脑怎样听懂指令开始，认识程序、代码、Python、运行和输出，再亲手完成第一行代码。', 'beginner', 1, '🌱', 80),
    ('变量、数据类型与运算符', '给信息贴上名字', '把变量理解成贴了标签的小盒子，从姓名和年龄等生活信息开始学习保存、修改和计算。', 'beginner', 2, '🧩', 110),
    ('条件判断', '教程序做选择', '从“下雨就带伞”开始理解条件，再逐步学习 if、elif 和 else，不要求提前认识英文关键字。', 'beginner', 3, '🚦', 100),
    ('循环结构', '让电脑重复做事', '从重复打印和数数开始，理解为什么需要循环，再学习 for、while 和停止条件。', 'basic', 4, '🔁', 120),
    ('字符串与常用容器', '整理一组信息', '用姓名单、购物清单和学生档案理解字符串、列表、字典等容器，再学习读取和修改。', 'basic', 5, '📦', 150),
    ('函数与模块', '制作自己的小工具', '从“把常用步骤装进一个按钮”理解函数，随后学习参数、返回值和模块复用。', 'basic', 6, '🛠️', 150),
    ('文件与异常处理', '保存结果并处理意外', '把文件理解成电脑里的笔记本，从保存一行文字开始，再学习读取和友好处理错误。', 'advanced', 7, '🗂️', 140),
    ('面向对象基础', '最后认识类和对象', '完成前面基础后，用“学生档案”和“游戏角色”理解类、对象、属性和方法。', 'advanced', 8, '🤖', 160)
)
...
with section_seed(chapter_title, title, summary, content_html, example_code, position, section_type) as (
  values
    ('认识 Python 与开发环境', '1.1 代码到底是什么', '从完全不知道代码开始，先认识电脑、程序、代码和 Python。', '<div class="zero-card"><b>开始前，你不需要会任何东西</b><p>如果你从来没见过代码，甚至不知道“运行”是什么意思，这正是这节课要解决的问题。先理解意思，不背单词。</p></div><h3>电脑很快，但不会猜</h3><p>电脑会严格执行人给它的指令。比如“显示一句欢迎语”“算出两个数字的和”“记住一个名字”，都需要把步骤说清楚。</p><div class="word-map"><div><b>程序</b><span>为了完成一件事而排好顺序的一组指令。</span></div><div><b>代码</b><span>我们用编程语言写下来的具体指令。</span></div><div><b>编程语言</b><span>人和电脑约定好的一套表达规则。</span></div><div><b>Python</b><span>一种比较容易阅读、适合初学者的编程语言。</span></div></div><h3>用做菜来理解</h3><p>菜谱像程序，每一步文字像代码，厨师照步骤做菜就像电脑运行代码。步骤写得越清楚，结果越稳定。</p><div class="checkpoint"><b>现在只要记住一句话：</b><p>代码就是写给电脑看的操作说明。Python 是写这种说明的一种语言。</p></div>', 'print("你好，Python！")', 1, 'lesson'),
    ('认识 Python 与开发环境', '1.2 第一次运行代码', '认识编辑区、运行按钮和结果区，逐字符完成第一行代码。', '<h3>“运行”是什么意思</h3><p>运行就是让电脑开始执行代码。写在编辑区里的代码不会自己工作，点击“运行”后，电脑才会读取并执行它。</p><div class="step-list"><div><i>1</i><span>找到页面里的代码编辑区，它像一张可以写指令的纸。</span></div><div><i>2</i><span>输入 <code>print("你好")</code>，注意英文括号和两个引号都要保留。</span></div><div><i>3</i><span>点击“运行”，然后观察下方结果区是否出现“你好”。</span></div><div><i>4</i><span>把“你好”改成自己的名字，再运行一次，观察什么发生了变化。</span></div></div><h3>拆开第一行代码</h3><p><code>print</code> 可以先理解为“请显示”；圆括号装着要交给它的内容；引号告诉 Python：这里是一段原样显示的文字。</p><div class="note">括号和引号不是装饰。少一个、多一个或使用了不成对的符号，都可能让电脑看不懂。</div>', 'print("你好，我开始学 Python 了！")', 2, 'lesson'),
    ('认识 Python 与开发环境', '1.3 写错了怎么办', '学会把报错当作电脑给出的修改提示，并完成第一次排错。', '<h3>写错是正常步骤</h3><p>编程不是一次写对，而是“写一点、运行、看结果、再修改”。专业开发者每天也会遇到报错。</p><h3>先检查最常见的三个地方</h3><div class="step-list"><div><i>1</i><span><b>拼写：</b><code>print</code> 是否写完整、全部小写？</span></div><div><i>2</i><span><b>符号：</b>左右括号和左右引号是否成对？</span></div><div><i>3</i><span><b>位置：</b>文字是否放在括号里面，并被引号包住？</span></div></div><h3>主动制造一个小错误</h3><p>先删除代码末尾的右括号再运行，观察错误提示；然后把右括号加回来再次运行。你刚刚完成了一次真正的调试。</p><div class="tip">现在不需要看懂全部英文报错。先知道它在提醒“某个位置无法理解”，再检查附近符号即可。</div><div class="checkpoint"><b>本节过关标准</b><p>你能说出代码、运行、输出的意思，并能修改 print 括号中的文字后看到新结果。</p></div>', 'print("写错不可怕，我会检查和修改。")', 3, 'lesson'),
...
with concept_seed(chapter_title, concepts) as (
  values
    ('认识 Python 与开发环境', '[{"term":"电脑指令","definition":"告诉电脑需要完成什么操作的清楚说明","keyword":"操作"},{"term":"程序","definition":"为了完成一件事而按顺序组织的一组指令","keyword":"一组指令"},{"term":"代码","definition":"用编程语言写给电脑看的操作说明","keyword":"操作说明"},{"term":"编程语言","definition":"人和电脑约定好的一套表达规则","keyword":"表达规则"},{"term":"Python","definition":"一种容易阅读、适合初学者的编程语言","keyword":"编程语言"},{"term":"运行","definition":"让电脑开始读取并执行代码","keyword":"执行"},{"term":"print()","definition":"让 Python 把指定内容显示出来","keyword":"显示"},{"term":"输出","definition":"程序运行后显示或产生的结果","keyword":"结果"},{"term":"引号与括号","definition":"print 代码中不可随意缺少的成对符号","keyword":"成对"},{"term":"调试","definition":"发现问题、检查代码并修改后再次运行的过程","keyword":"修改"}]'::jsonb),
...
with concept_seed(chapter_title, concepts) as (
  select title, case position
    when 1 then '[{"term":"电脑指令","keyword":"操作"},{"term":"程序","keyword":"一组指令"},{"term":"代码","keyword":"操作说明"},{"term":"编程语言","keyword":"表达规则"},{"term":"Python","keyword":"编程语言"},{"term":"运行","keyword":"执行"},{"term":"print()","keyword":"显示"},{"term":"输出","keyword":"结果"},{"term":"引号与括号","keyword":"成对"},{"term":"调试","keyword":"修改"}]'
